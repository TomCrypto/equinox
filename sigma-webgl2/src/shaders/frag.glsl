...
bool ray_bvh(vec3 origin, vec3 direction, out Result result) {
    ...
    while (offset != bvh_limit) {
        ...
        if (ray_bbox(origin, inv_dir, elem1.xyz, elem2.xyz)) {
            ...
            if (data != uint(0)) {
                ...
                read_triangle(data - uint(1), triangle);
...
bool ray_bvh_occlusion(vec3 origin, vec3 direction) {
    vec3 inv_dir = vec3(1.0) / direction;
...
void main() {
    ...
    // intersect the BVH
    if (ray_bvh(pos, dir, result) && result.distance > 0.0) {
