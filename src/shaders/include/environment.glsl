...
vec3 env_sample_light(out vec3 wi, out float pdf, inout quasi_t quasi) {
    if (environment.has_envmap == 1) {
        return env_sample_light_image(wi, pdf, quasi);
    } else {
        return env_sample_light_solid(wi, pdf, quasi);
    }
}
